...
from get_prob_util import ALL_POOL, pronostics, BOUNDS
...
print(f"** Game - list of prob - average prob **")
for k, v in whisk.items():
    formatted_data = ', '.join([f'{value:.2f}' for value in v])
    av = np.average(v)
    print(f"{k} [{formatted_data}] {av:.2f}")
